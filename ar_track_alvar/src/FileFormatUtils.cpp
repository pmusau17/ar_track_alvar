...
namespace alvar {
	...
	bool FileFormatUtils::decodeXMLMatrix(const tinyxml2::XMLElement  *xml_matrix, int &type, int &rows, int &cols) {
		...
		if (xml_matrix->QueryIntAttribute("rows", &rows) != EXIT_SUCCESS) return false;
		if (xml_matrix->QueryIntAttribute("cols", &cols) != EXIT_SUCCESS) return false;

		return true;
	}

	CvMat* FileFormatUtils::allocateXMLMatrix(const tinyxml2::XMLElement  *xml_matrix) {
	...
	bool FileFormatUtils::parseXMLMatrix(const tinyxml2::XMLElement  *xml_matrix, CvMat *matrix) {
		...
		for (int r = 0; r < matrix->rows; ++r) {
			for (int c = 0; c < matrix->cols; ++c) {
				...
				xml_data = (const tinyxml2::XMLElement *) xml_data->NextSiblingElement("data");
			}
		}
	...
	tinyxml2::XMLElement * FileFormatUtils::createXMLMatrix(const char* element_name, const CvMat *matrix) {
		if (!matrix) return NULL;

		tinyxml2::XMLElement * xml_matrix;
		xml_matrix->SetName(element_name);
		int precision;
		...
		for (int r = 0; r < matrix->rows; ++r) {
			for (int c = 0; c < matrix->cols; ++c) {
				tinyxml2::XMLElement  *xml_data;
				xml_data->SetName("data");
